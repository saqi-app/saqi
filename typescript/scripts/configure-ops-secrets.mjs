...
function validSourceProfile(raw) {
  ...
  if (
    !hasExactFields(profile, ["dom", "feed", "labels", "routes", "schemaVersion"]) ||
    profile.schemaVersion !== 1
  ) return false;
  if (
    !hasExactFields(profile.dom, SOURCE_PROFILE_FIELDS.dom) ||
    !SOURCE_PROFILE_FIELDS.dom.every((field) => isBoundedText(profile.dom[field]))
  ) return false;
  if (
    !hasExactFields(profile.labels, SOURCE_PROFILE_FIELDS.labels) ||
    !SOURCE_PROFILE_FIELDS.labels.every((field) => isLexemeList(profile.labels[field]))
  ) return false;
  ...
  if (
    ...
    !["cursorParameter", "endpointMarker", "responseHtmlKey", "responseNextCursorKey", "tokenParameter"].every((field) => isBoundedText(profile.feed[field], 128))
  ) return false;
  if (!hasExactFields(profile.routes, SOURCE_PROFILE_FIELDS.routes)) return false;
...
function parseProfile(raw) {
  try {
    ...
    return { ok: false, error: "INVALID_SOURCE_PROFILE_JSON" };
  }
}
...
function isBoundedText(value, maximum = 512) {
  return String(value) === value && value.trim().length > 0 && value.length <= maximum;
}
