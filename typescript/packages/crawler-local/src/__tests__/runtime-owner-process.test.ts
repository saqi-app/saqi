...
test("two processes cannot both claim or remove a replacement runtime owner", async () => {
  ...
  database.exec(
    "CREATE TABLE local_schema(singleton INTEGER PRIMARY KEY, version INTEGER); INSERT INTO local_schema VALUES(1,39)",
  );
  database.exec(RUNTIME_OWNER_MIGRATION_SQL);
  const base = import.meta.dirname;
  for (const name of ["runtime-owner-schema", "runtime-owner-store"]) {
