...
function pass(): AuthorInventoryPageProjection[] {
  return [
    {
      authors: [
        { href: "/cat-b", name: "باء", poemCountText: "٢" },
        { href: "/cat-a", name: "ألف", poemCountText: null },
      ],
      challengeDetected: false,
      kind: "author_inventory_page",
      nextPageHref: "/authers-2",
      page: 1,
      schemaVersion: 1,
      sourceUrl: "/authers-1",
      terminal: false,
    ...
    {
      authors: [
        { href: "/cat-b", name: "باء", poemCountText: "2" },
        { href: "/cat-c", name: "جيم", poemCountText: "١" },
      ],
      challengeDetected: false,
      kind: "author_inventory_page",
      nextPageHref: null,
...
describe("author inventory page certificate", () => {
  ...
  it("normalizes source presentation noise without losing real authors", async () => {
    ...
    const certificate = await certifyAuthorInventoryPages(noisy, noisy);
    expect(certificate.authors).toContainEqual(
      expect.objectContaining({
        href: "https://source.invalid/cat-poet-%E2%80%8EAhmed-Al-Luwaim",
        name: "أحمد اللويم",
        poemCount: 6,
      }),
    );
